...
export function Btn({ children, variant = 'primary', color, small, onClick, disabled = false, type = 'button' }: {
  children: ReactNode; variant?: 'primary' | 'secondary' | 'ghost'; color?: string; small?: boolean; onClick?: () => void; disabled?: boolean; type?: 'button' | 'submit'
}) {
  const base: React.CSSProperties = {
    display: 'inline-flex', alignItems: 'center', gap: 5,
    borderRadius: 5, cursor: disabled ? 'not-allowed' : 'pointer', fontWeight: 600, border: '1px solid transparent',
  ...
  return <button type={type} disabled={disabled} style={{ ...base, ...variants[variant], opacity: disabled ? 0.55 : 1 }} onClick={onClick}>{children}</button>
}
